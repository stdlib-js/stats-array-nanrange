...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( nanrange.length, 1, 'returns expected value' );
	t.end();
});
...
tape( 'if provided an empty array, the function returns `NaN` (accessors)', function test( t ) {
	var v = nanrange( toAccessorArray( [] ) );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
	t.end();
});

tape( 'if provided an array containing a single element, the function returns the zero', function test( t ) {
	var v = nanrange( [ 1.0 ] );
	t.strictEqual( v, 0.0, 'returns expected value' );
	t.end();
});

tape( 'if provided an array containing a single element, the function returns the zero (accessors)', function test( t ) {
	var v = nanrange( toAccessorArray( [ 1.0 ] ) );
	t.strictEqual( v, 0.0, 'returns expected value' );
	t.end();
});
